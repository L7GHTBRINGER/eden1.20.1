...
public class GravehagEntity extends Monster implements GeoEntity {
    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    public GravehagEntity(EntityType<? extends Monster> entityType, Level level) {
    ...
    public boolean doHurtTarget(Entity opfer) {
        if (super.doHurtTarget(opfer)) {
            if (opfer instanceof LivingEntity) {
                ((LivingEntity) opfer).addEffect(new MobEffectInstance(MobEffects.BLINDNESS, 200), this);
            }
        }
        return true;
    }
    ...
    public GravehagEntity getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return ModEntities.GRAVEHAG.get().create(level);
    }
