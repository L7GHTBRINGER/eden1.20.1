...
public class SwamphagEntity extends Monster implements GeoEntity {
    ...
    public boolean doHurtTarget(Entity opfer) {
        if (super.doHurtTarget(opfer)) {
            if (opfer instanceof LivingEntity) {
                ((LivingEntity) opfer).addEffect(new MobEffectInstance(MobEffects.POISON, 40), this);
            }
        }
        return true;
    }
    ...
    public SwamphagEntity getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return ModEntities.SWAMPHAG.get().create(level);
    }
