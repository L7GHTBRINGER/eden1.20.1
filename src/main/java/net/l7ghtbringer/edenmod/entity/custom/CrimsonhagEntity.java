...
public class CrimsonhagEntity extends Monster implements GeoEntity {
    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    public CrimsonhagEntity(EntityType<? extends Monster> entityType, Level level) {
    ...
    public boolean doHurtTarget(Entity opfer) {
        if (super.doHurtTarget(opfer)) {
            if (opfer instanceof LivingEntity) {
                ((LivingEntity) opfer).setSecondsOnFire(5);
            }
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(3, new NearestAttackableTargetGoal<>(this, WarpedhagEntity.class, true));

    }

    public CrimsonhagEntity getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return ModEntities.CRIMSONHAG.get().create(level);
    }
