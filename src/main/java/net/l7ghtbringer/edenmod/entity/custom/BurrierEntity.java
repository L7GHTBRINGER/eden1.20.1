...
public class BurrierEntity extends Monster implements GeoEntity {
    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    public BurrierEntity(EntityType<? extends Monster> entityType, Level level) {
    ...
    public boolean doHurtTarget(Entity opfer) {
        if (super.doHurtTarget(opfer)) {
            if (opfer instanceof LivingEntity) {
                ((LivingEntity) opfer).addEffect(new MobEffectInstance(MobEffects.BLINDNESS, 100), this);
            }
        }
        return true;
    }
    ...
    public BurrierEntity getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return ModEntities.BURRIER.get().create(level);
    }
